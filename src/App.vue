...
const updateLog = {
  ...
  "1.1.0": "支持调整字体大小，显示课程节数，更新课程表 【25/02/12打包 未实装】",
  "1.1.1": "调字体大小改为右键头 【25/02/12打包 25/02/13实装】",
}
...
function getTodayClass(): Array<string> {
  ...
  return config.classtable[new Date().getDay().toString() as '1' | '2' | '3' | '4' | '5'];
}
...
<template>
  <div class="container" data-tauri-drag-region>
    ...
    <div v-if="showFontSizePopup" class="popup">
      ...
      <br>
      <div class="about">
        <b>作者：刘宇轩 版本：{{ currentVersion }}</b>
        <p>更新日志：</p>
        <p>{{ updateLog[currentVersion] }}</p>
      </div>
    </div>
  </div>
</template>
